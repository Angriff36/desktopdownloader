...
class _DownloaderScreenState extends State<DownloaderScreen> {
  ...
  dynamic platform; // Late initialization only on mobile
  ...
  void initState() {
    ...
    if (Platform.isAndroid || Platform.isIOS) {
      _initializeMobileSharing();
    }
  }

  void _initializeMobileSharing() async {
  ...
  Future<void> downloadVideo() async {
    ...
    final backendEndpoint = "http://localhost:8000/download?url=$url";

    try {
      final response =
          await http.Client().send(http.Request('GET', Uri.parse(backendEndpoint)));
      ...
      if (response.statusCode == 200) {
        ...
        int receivedBytes = 0;
        List<int> bytes = [];
        final downloadDir = Directory(Platform.isWindows
        ...
        response.stream.listen(
          (List<int> chunk) {
            ...
            if (response.contentLength != null) {
              setState(() {
                _downloadProgress = receivedBytes / response.contentLength!;
              });
            }
          },
          onDone: () async {
            ...
            ElevatedButton(
              onPressed: () async {
                if (Platform.isAndroid) {
                  ...
                  setState(() {
                    status = "Gallery feature is only available on mobile.";
                  });
                }
              },
